function GamePolicy() {
  ...
  let player1TotalScore = new Score(
    new Vector2(Game.size.x / 2 - 75, Game.size.y / 2 - 45)
  )
  let player2TotalScore = new Score(
    new Vector2(Game.size.x / 2 + 75, Game.size.y / 2 - 45)
  )
  ...
  this.players = [
    new Player(player1MatchScore, player1TotalScore),
...
GamePolicy.prototype.checkColisionValidity = function (ball1, ball2) {
  ...
  if (
    this.players[this.turn].matchScore.value == 7 &&
...
GamePolicy.prototype.isOutsideBorder = function (pos, origin) {
  return (
    this.isXOutsideLeftBorder(pos, origin) ||
    this.isXOutsideRightBorder(pos, origin) ||
    this.isYOutsideTopBorder(pos, origin) ||
    this.isYOutsideBottomBorder(pos, origin)
  )
}
...
GamePolicy.prototype.isInsideHole = function (pos) {
  return (
    this.isInsideTopLeftHole(pos) ||
    this.isInsideTopRightHole(pos) ||
    this.isInsideBottomLeftHole(pos) ||
    this.isInsideBottomRightHole(pos) ||
